...
- 足りない gem があれば `gem install` を提案する。ベンチなら `benchmark-ips` 相当、無ければ stdlib `Benchmark.realtime` でも十分。

### 1. ブランチと作業ディレクトリを切る

```sh
git checkout -b work/<theme>-<short-topic>
mkdir -p work/<theme>
```

- ブランチ名は `work/` プレフィクスで統一(repo の慣習)。
...
- `git log --oneline` で 1 実験 1 コミットになっているか点検する。
- feedback のコミットは最後に単独で。
- PR にするかは都度相談。**FEEDBACK.md をガイドの改善 PR 材料**に使う流れも視野に。
