...
## 06. RString のメモリレイアウト観察 (VWA)

`ObjectSpace.memsize_of("a" * N)` を N を変えて並べた。Ruby 4.0.2。
...
気づき:
- 「embedded(構造体内)」と「heap(別バッファ)」の**二値ではなかった**。Ruby 4.0 は **VWA (Variable Width Allocation)** で、GC スロット自体が **40 / 80 / 160 / 320 / 640 bytes** といった複数サイズクラスを持ち、短い文字列ほど小さいスロットに収まる。
- 40 → 80 の最初のジャンプは **16 bytes 付近**。12 bytes までは 40 byte の基底スロットに埋め込める(古典的な `RSTRING_EMBED_LEN_MAX == 23` という 7 年前の知識はそのままだと合わない)。
- 1024 bytes で `memsize_of = 1065`。ここだけ「サイズクラスの倍々」から外れて「実際のバイト数 + 小さなヘッダ」になる。ここが**真の意味で heap 確保**(VWA スロットに収まらず `malloc` に落ちる)になっていると推測。
- `dup`, `-@`, `+@` は 64 bytes の base で全部 `memsize_of = 160` と同じ値。frozen かどうか・interning されているかは `memsize_of` からは見えない。
- `big[0, 500]` は 640 bytes のスロットに収まる大きさ。共有バッファの恩恵は `memsize_of` 単体では分かりにくいので、`ObjectSpace.dump` や `rb_str_shared_p` を見る必要がありそう(この先は C 側に踏み込むテーマ)。

#### ガイドの記述と現実のズレ

`themes/string.md` の「Embedded String と Heap String の境界を自分の目で確かめる」は、VWA 導入前のメンタルモデル寄り。実際は **embedded / heap の 2 値ではなく、複数段階のサイズクラス → 最終的に真の heap** という 3 層以上の構造で、**最初のジャンプは 23 bytes ではなく 12〜16 bytes あたり**になる。この観察は Kaigi 本編で Variable Width Allocation や ko1 の RString 最適化トークを聞く時の前提知識として刺さりそう。
