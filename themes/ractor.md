...
- CPU バウンドな処理(例: フラクタル描画・素数列挙・画像変換)を Ractor で並列化して、Thread 版とのスループットを比較する(純 Ruby の CPU 処理では GVL のため **Thread は直列実行より遅くなる**のが典型。「速くならない」ではなく「負ける」数字が出るはず、を事前に知っておくと結果を読み違えない)
